...
class FridayRouter:
    def __init__(self):
        ...
        self.no_llm_mode = True # Force intelligence to be purely procedural
        self.llm = None
        self.reasoning_agent = None
        logger.info("Friday is running on internal procedural logic. LLMs disabled.")
    ...
    async def route(self, text: str) -> str:
        ...
        # ----------------------------------------------------------------
        # ----------------------------------------------------------------
        # Fast built-in handlers (no LLM needed) - Expanded Conversational Logic
        # ----------------------------------------------------------------
        ...
        # Catching farewells and casual sign-offs
        if any(p in normalized for p in ["see you", "goodbye", "bye friday", "exit", "quit", "talk to you later"]):
            if "evening" in normalized:
                return "Understood. Have a wonderful evening! I will be here when you need me."
            if "later" in normalized or "tomorrow" in normalized:
                return "Alright then. Catch you later!"
            return "Goodbye! Shutting down voice listening systems."
    ...
    async def _route_without_llm(self, text: str, normalized: str) -> str:
        ...
        # ==========================================
        # 1. DYNAMIC WEB & MEDIA ENGINE
        # ==========================================
        # Captures: "play [song name]" or "play [song name] on youtube"
        play_match = re.search(r"play\s+(.+?)(?:\s+on\s+youtube)?$", normalized)
        if play_match:
            song = play_match.group(1).strip()
            return await asyncio.to_thread(open_youtube, song, True)

        # Captures: "search google for [query]" or "search [query]"
        search_match = re.search(r"search\s+(?:google\s+for\s+|for\s+)?(.+)", normalized)
        if search_match and "youtube" not in normalized and "file" not in normalized:
            query = search_match.group(1).strip()
            return await asyncio.to_thread(search_google, query)

        # ==========================================
        # 2. ADVANCED FILE OPERATIONS (REGEX POWERED)
        # ==========================================
        # Captures: "move [file/folder] to [destination]" or "move the file [file] to [dest]"
        move_match = re.search(r"move\s+(?:the\s+file\s+)?(.+?)\s+to\s+(.+)", normalized)
        if move_match:
        ...
        elif "move file" in normalized and " to " in normalized:
            source, target = normalized.split(" to ", 1)
            source = source.replace("move file", "", 1).strip()
            target = target.strip()
            return await asyncio.to_thread(move_file, source, target)
        ...
        # ==========================================
        # 5. SYSTEM ANALYTICS & BASIC FILES
        # ==========================================
        if "battery" in normalized or "cpu" in normalized or "memory" in normalized or "system report" in normalized:
            raw = await asyncio.to_thread(get_system_report)
        ...
        return (
            "I am running in pure procedural mode. "
            "I can handle system controls, web searches, file operations, and remembered preferences natively. "
            "Try asking something like 'play AC/DC', 'search Google for Python tutorials', or 'remember my preference is dark theme'."
        )
